...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros . induction l as [| l' m' n'].
  - simpl. reflexivity.
  - simpl. rewrite ->  n'. reflexivity. Qed.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun a => evenb a && negb (blt_nat a 7)) l.
...
Definition partition {X : Type}
                     ...
                     (l : list X)
                   : list X * list X :=
  (* This is more easily expressed iwth foldr, I don't think with just filter
  you can do this in one pass like you could with foldr *)
...
Lemma map_comp : forall (X Y : Type) (x : X) (f : X -> Y) (l : list X),
...
Proof.
  intros. induction l as [].
  - simpl. reflexivity.
  - simpl. rewrite <- IHl. rewrite -> map_comp. reflexivity. Qed.
(** [] *)
...
(* Yes, having more generality will always be more useful. For example, if X and
Y were not different then implementing map via fold would be impossible, at
least the map we want (that is one that is a functor and not _necessarily_
an endofunctor)*)
(** [] *)
