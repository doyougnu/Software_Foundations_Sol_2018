...
(*     (b \\\ b') -> (BNot b) \\\ (negb b') *)
| E_BNot : forall (b : bexp) (b' : bool),
    (bevalR b b') -> bevalR (BNot b) (negb b')
...
Proof.
  ...
  (* The Not case *)
  repeat (try (simpl; rewrite IHbevalR; reflexivity)).
  try (simpl; rewrite IHbevalR1; rewrite IHbevalR2; reflexivity).
  - intros. generalize dependent bv;
    induction b; simpl; intros; subst;
    repeat (try (constructor; apply aeval_iff_aevalR)).
    (* repeat (try (constructor; repeat assumption; reflexivity)). *)
    (* WHY CAN"T WE USE ASSUMPTION HERE ?!?!?!*)
    + constructor. apply IHb. reflexivity.
    + constructor. apply IHb1. reflexivity.
      * apply IHb2. reflexivity. Qed.
