...
Proof.
  apply nat_ind.
  - (* n = 0 *) trivial.
  - (* n = S n' *) simpl. intros. rewrite <- H. reflexivity. Qed.
...
Inductive rgb : Type :=
  ...
  | green : rgb
  | blue : rgb.
(* rgb_ind : forall P : rgb -> P, P red -> P green -> P blue -> forall y : rgb, P y*)
Check rgb_ind.
(** [] *)
...
(forall (y : yesno), P y -> P (bleaf y)) ->
(forall (z : yesno) (a : byntree) (b : byntree), P a -> P b -> P (nbranch z a b))] *)
...
(** [tree_ind : forall (X : Type) (P : tree X -> Prop),
    (forall (x : X), P (leaf x)) ->
    (forall (l : Tree X), P l -> (forall (r : Tree X), P r -> P (node l r)))] *)
...
Definition plus_assoc'' (n m k : nat) : Prop :=
  n + (m + k) = (n + m) + k.

Definition plus_comm2 (n m : nat) : Prop :=
  n + m = m + n.
...
Theorem plus_comm''' : forall (n m : nat),
  ...
  Proof.
    induction m as [].
    - (*m = 0*) unfold plus_comm2. simpl. rewrite <- plus_n_O. reflexivity.
    - (*m = S m' *) unfold plus_comm2 in IHm.
      unfold plus_comm2. simpl. rewrite <- IHm. rewrite <- plus_n_Sm. reflexivity.
      Qed.
...
Proof.
  ...
  - unfold plus_assoc''. simpl. trivial.
  - unfold plus_assoc'' in IHn'. unfold plus_assoc''. simpl. rewrite IHn'.
    reflexivity. Qed.
